...
import type {
    ...
    Recommendation, SystemHealth, Responder,
    AIRecommendation, TimelineEvent,
} from '../types/index.js';
...
const buildIncident = (): Incident => ({
    ...
    detectedAt: ago(7), updatedAt: ago(1),
    confidence: 87, vehiclesAffected: 134,
    avgDelay: 18, congestionIndex: 78, clearanceEta: 22,
    recommendations: [
        { id: 'r1', type: 'route',    action: 'Redirect via Mbagathi Way',    detail: 'Alternate route reduces queue by 22% over 15 min', confidence: 91, impact: '−22% congestion', eta: '2 min',  status: 'pending'  },
        ...
        { id: 'r3', type: 'signal',   action: 'Adjust signals — 3 junctions', detail: 'Extend green on Mbagathi, Langata, James Gichuru',confidence: 84, impact: '+15% throughput', eta: '1 min',  status: 'pending'  },
    ] as AIRecommendation[],
    responders: [
...
export const incidents = new Map<string, Incident>([
    ['INC-2026-0847', buildIncident()],
]);
...
export const recommendations = new Map<string, Recommendation>([
    ['rec-1', { id: 'rec-1', title: 'Redirect via Mbagathi Way',   description: 'Reduce CBD Ring Road congestion by 22%', confidence: 91, impact: '−22% congestion', type: 'route',   status: 'pending', createdAt: ago(3),  alertId: 'a1' }],
    ['rec-2', { id: 'rec-2', title: 'Extend green — 3 junctions', description: 'Improve throughput by 15% on Ngong Rd', confidence: 84, impact: '+15% throughput', type: 'signal',  status: 'pending', createdAt: ago(6),  alertId: 'a5' }],
    ['rec-3', { id: 'rec-3', title: 'Deploy tow truck KCC-03',     description: 'Clear Thika Rd within 8 min',          confidence: 96, impact: 'Clears lane in 8m', type: 'dispatch',status: 'approved',createdAt: ago(7),  alertId: 'a2' }],
]);

// ─── System health store ──────────────────────────────────────────────────────

export const systemHealth: SystemHealth = {
    overall: 'ok', sensors: 248, activeSensors: 234, latencyMs: 42, uptime: 99.4,
    metrics: [
