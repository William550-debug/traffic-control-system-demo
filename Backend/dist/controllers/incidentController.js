...
// ─── HANDLERS ────────────────────────────────────────────────────────────────
export const getIncidents = (_req, res) => {
...
export const confirmIncident = (req, res) => {
    const id = req.params.id;
    const inc = incidents.get(id);
    if (!inc)
        return res.status(404).json({ ok: false, error: 'Incident not found' });
...
export const approveRecommendation = (req, res) => {
    ...
    if (!inc)
        return res.status(404).json({ ok: false, error: 'Incident not found' });
    const rec = inc.recommendations.find((r) => r.id === recId);
    if (!rec)
...
export const rerouteTraffic = (req, res) => {
    const id = req.params.id;
    const inc = incidents.get(id);
    if (!inc)
        return res.status(404).json({ ok: false, error: 'Incident not found' });
...
export const adjustSignals = (req, res) => {
    const id = req.params.id;
    const inc = incidents.get(id);
    if (!inc)
        return res.status(404).json({ ok: false, error: 'Incident not found' });
...
export const dispatchResponder = (req, res) => {
    const id = req.params.id;
    const rspId = req.params.rspId;
    const inc = incidents.get(id);
    if (!inc)
        return res.status(404).json({ ok: false, error: 'Incident not found' });
    const rsp = inc.responders.find(r => r.id === rspId);
    if (!rsp)
        return res.status(404).json({ ok: false, error: 'Responder not found' });
...
export const updateResponderRoute = (req, res) => {
    const id = req.params.id;
    const rspId = req.params.rspId;
    const inc = incidents.get(id);
    if (!inc)
