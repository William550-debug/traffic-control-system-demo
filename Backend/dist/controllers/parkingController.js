...
function logZoneAction(operator, zone, action, type, // matches ParkingAuditLog.type
aiSuggestion) {
...
export const getZones = (_req, res) => {
    try {
...
export const getRecommendations = (_req, res) => {
    try {
...
export const approveRecommendation = (req, res) => {
    try {
        ...
        const response = { recommendation: { ...rec, status: 'approved' } };
        if (updatedZone)
            response.zone = updatedZone;
        res.json(response);
    }
    catch (error) {
        console.error(error);
