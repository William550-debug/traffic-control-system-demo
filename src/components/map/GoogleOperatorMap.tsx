...
const CORRIDOR_LINES = [
    {
        id: 'corridor-001', name: 'CBD Ring Road', status: 'blocked' as CorridorStatus,
        path: [[-1.2832,36.8172],[-1.2856,36.8195],[-1.2880,36.8220],[-1.2910,36.8235],[-1.2935,36.8218],[-1.2950,36.8192]],
    },
    {
        id: 'corridor-002', name: 'Uhuru Highway', status: 'congested' as CorridorStatus,
        path: [[-1.2820,36.8163],[-1.2870,36.8163],[-1.2930,36.8163],[-1.2990,36.8163],[-1.3060,36.8163]],
    },
    {
        id: 'corridor-003', name: 'Ngong Road', status: 'congested' as CorridorStatus,
        path: [[-1.2920,36.8050],[-1.2960,36.7970],[-1.3010,36.7900],[-1.3010,36.7840],[-1.3010,36.7770]],
    },
    {
        id: 'corridor-004', name: 'Thika Superhighway', status: 'emergency' as CorridorStatus,
        path: [[-1.2750,36.8290],[-1.2700,36.8320],[-1.2660,36.8350],[-1.2631,36.8370],[-1.2590,36.8410]],
    },
    {
        id: 'corridor-005', name: 'Mombasa Road', status: 'normal' as CorridorStatus,
...
function OperatorOverlays({
    ...
    const corridorPolylinesRef = useRef<google.maps.Polyline[]>([]);
    const corridorGlowRef      = useRef<google.maps.Polyline[]>([]);
    ...
    // ── Sync map center/zoom from mapState ────
    useEffect(() => {
        if (!map) return;
    ...
    useEffect(() => {
        ...
        corridorPolylinesRef.current.forEach(p => p.setMap(null));
        corridorGlowRef.current.forEach(p => p.setMap(null));
        corridorLabelsRef.current.forEach(m => m.setMap(null));
        corridorPolylinesRef.current = [];
        corridorGlowRef.current = [];
        ...
        CORRIDOR_LINES.forEach(corridor => {
            ...
            if (onCorridorClick) {
                ...
                line.set('cursor', cursor);
                glow.set('cursor', cursor);
            }

            // Label at midpoint
            const mid   = corridor.path[Math.floor(corridor.path.length / 2)];
            const label = new google.maps.Marker({
                position: { lat: mid[0] - 0.003, lng: mid[1] },
                map,
            ...
            corridorGlowRef.current.push(glow);
    ...
    useEffect(() => {
        ...
        visible.forEach(alert => {
            ...
            <div style="
              background:#0c1117; color:#e2e8f0;
              padding:10px 12px; border-radius:6px;
              font-family:monospace; font-size:11px;
              border:1px solid ${color}40; max-width:220px;
            ">
              <div style="color:${color}; font-weight:700; margin-bottom:4px; font-size:12px;">
                ${alert.title}
              </div>
              <div style="color:#718096; margin-bottom:6px; line-height:1.5;">
                ${alert.description}
              </div>
    ...
    }, [map, mapsLib, alerts, showIncidents, onAlertClick]);
    ...
    return (
        <APIProvider apiKey={apiKey} libraries={['visualization']}>
            <Map
                ...
                onCenterChanged={e => onCenterChange(
                    e.detail.center.lat,
                    e.detail.center.lng,
                )}
            >
